...
const SWAP_ROOT: ConstSizedMatrix<2, Complex> = ConstSizedMatrix::new([
...
impl_operator!(SWAP_ROOT, Complex, Complex, SwapRoot, crate::toolbox::rotation::swap::RotationSwap, 2, 2.0, "");
...
mod test {
    ...
    fn test_h_swap() {
        ...
        let ket = SwapRoot::new(0, 1).apply(ket, &mut register);
        assert_approx_eq!(f64, ket.probability(0), 0.25, epsilon = 0.00000003);
        assert_approx_eq!(f64, ket.probability(1), 0.25, epsilon = 0.00000003);
        let ket = SwapRoot::new(0, 1).apply(ket, &mut register);
